...
| Trivy `ignore-unfixed` on `pixeldive:ci` | Debian bookworm CVEs in `python:3.13.15-slim-bookworm` with `will_not_fix` / `fix_deferred` (zlib, perl, sqlite, util-linux, ncurses). No in-repo patch exists; Dependabot digest bumps pick up Debian fixes ([DOCKER-BUILD-006](https://github.com/pirlruc/guardrails/blob/1.6.0/docker/guardrails.md)). Fixable Python CVEs still fail the job. |
