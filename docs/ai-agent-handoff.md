...
- Draft Cursor PRs may not start Actions until a later `synchronize` push; GitHub also holds runs when a PR edits workflow files until a maintainer approves them.
...
- Follow-up: production resolves HTTP and gRPC PEMs (not flags only); `GrpcClient` refuses PEMs on insecure channels; HEALTHCHECK loads mTLS client identity
