...
| Host may lack `python3-venv` | Use `pip install --target .venv` or `pip install --prefix .venv` (Debian: `.venv/local/...`). `scripts/python-env.sh` resolves both layouts. Do not install into the system OS. `pip-audit` in `ci-local.sh` uses `--no-deps --disable-pip` so it does not spawn a throwaway venv. |
