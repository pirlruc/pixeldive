...
| Auth is opt-in | `AUTH_REQUIRED` defaults to false so local tests and compose stay unauthenticated. Set `ENVIRONMENT=production` (requires `AUTH_REQUIRED`, `HTTP_INSECURE=false`, and `GRPC_INSECURE=false`) plus a PEM pair. Host-native HTTP/gRPC binds default to `127.0.0.1`; the image/Compose set `0.0.0.0` inside the container network. |
