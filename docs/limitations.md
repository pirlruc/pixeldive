...
| Swift / Xcode not on Linux CI hosts | SWIFT-ENV-001 requires macOS for UIKit/`xcodebuild`. `scripts/check-ios-sdk.sh` skips when `swift` is missing unless `PIXELDIVE_REQUIRE_SWIFT=1`. The `ios-sdk` job runs on `macos-15`. Analog proposal: [new-guardrails/swift.md](new-guardrails/swift.md). |
