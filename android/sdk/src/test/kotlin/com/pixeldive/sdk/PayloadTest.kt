...
class PayloadTest {
    ...
    fun sanitizerAndPageQuery() {
        ...
        assertEquals("image/png", sanitizeMultipartType("image/png; charset=utf-8"))
        assertEquals("application/octet-stream", sanitizeMultipartType("nope"))
        assertEquals("a_.png", sanitizeMultipartFilename("a\u0000.png"))
        assertEquals(2048, clippedUtf8(ByteArray(4096) { 'x'.code.toByte() }).length)
